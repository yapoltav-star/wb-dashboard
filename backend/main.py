...
# Временный зонд для разбора поисковых ключей (без UI). Удалить после анализа.
@app.get("/api/_tmp_search_probe")
def tmp_search_probe(days: int = 270, nm_id: int = 0, mode: str = "probe"):
    """
    mode=probe — сырой ответ WB (форма JSON).
    mode=agg — агрегат ключей по корзине/заказам за период.
    """
    if not WB_TOKEN:
        return {"error": "WB_TOKEN не задан"}
    days = max(7, min(int(days or 270), 365))
    end = datetime.now(timezone.utc).date()
    start = end - timedelta(days=days - 1)
    # «3 квартала» 2026: с 1 янв, если days>=180
    if days >= 180:
    ...
    def _row_from_text(obj, nm=None):
        if not isinstance(obj, dict):
            return None
        text = obj.get("text") or obj.get("searchText") or obj.get("query") or obj.get("keyword")
        if not text or not isinstance(text, str):
            return None
        m = obj.get("metrics") if isinstance(obj.get("metrics"), dict) else obj
        return {
            "text": text.strip(),
            "frequency": int(_cur(m.get("frequency"))),
            "avg_position": _cur(m.get("avgPosition")),
            "open_card": int(_cur(m.get("openCard"))),
            "add_to_cart": int(_cur(m.get("addToCart"))),
            "orders": int(_cur(m.get("orders"))),
            "nm_id": nm or obj.get("nmId") or obj.get("nmID"),
        }

    def _walk(obj, acc, nm=None):
        if isinstance(obj, dict):
            # вложенные списки текстов
            for key in ("searchTexts", "texts", "queries", "topSearchTexts", "items"):
                val = obj.get(key)
                if isinstance(val, list):
                    local_nm = obj.get("nmId") or obj.get("nmID") or nm
                    for it in val:
                        r = _row_from_text(it, local_nm)
                        if r:
                            acc.append(r)
                        else:
                            _walk(it, acc, local_nm)
            r = _row_from_text(obj, nm)
            if r:
                acc.append(r)
            for k, v in obj.items():
                if k in ("searchTexts", "texts", "queries", "topSearchTexts"):
                    continue
                _walk(v, acc, obj.get("nmId") or obj.get("nmID") or nm)
    ...
    if nm_id:
        ...
        try:
            since = (datetime.now(timezone.utc) - timedelta(days=100)).strftime("%Y-%m-%d")
            resp = httpx.get(
                f"{SUPABASE_URL}/rest/v1/article_daily_stats?dt=gte.{since}&select=nm_id,orders&limit=8000",
            ...
            if resp.is_success:
                ...
                nms = [x for x, _ in sorted(agg.items(), key=lambda z: -z[1])[:25]]
        except Exception as e:
            logger.warning(f"tmp search nm list: {e}")

    if mode == "probe":
        out = {"period": period, "nms": nms[:5]}
        try:
            r = httpx.post(
            ...
            out["report_status"] = r.status_code
            out["report_body"] = r.text[:4000]
        except Exception as e:
        ...
        if nms:
            try:
                ...
                out["product_status"] = r.status_code
                out["product_body"] = r.text[:4000]
            except Exception as e:
    ...
    try:
        for field in ("orders", "addToCart"):
            ...
            if r.is_success:
                ...
                warnings.append(f"report/{field}: {r.status_code} {r.text[:160]}")
            time.sleep(21)
    except Exception as e:
        warnings.append(f"report: {e}")

    # product texts
    for top_by in ("orders", "addToCart"):
        for i in range(0, min(len(nms), 15), 3):
            batch = nms[i:i + 3]
            try:
                r = httpx.post(
                    f"{WB_ANALYTICS_URL}/api/v2/search-report/product/search-texts",
                ...
                if r.is_success:
                    ...
                    warnings.append(f"product/{top_by}: {r.status_code} {r.text[:160]}")
                    if r.status_code == 400:
                        break
            except Exception as e:
                warnings.append(f"product/{top_by}: {e}")
            time.sleep(21)
    ...
    for r in rows:
        ...
        if not t:
            continue
        cur = by_text.setdefault(t, {
            "text": r["text"].strip(),
            "orders": 0, "add_to_cart": 0, "open_card": 0,
            "frequency": 0, "avg_position_sum": 0.0, "avg_position_n": 0,
        })
    ...
    by_orders = sorted([q for q in queries if q["orders"] > 0], key=lambda x: -x["orders"])
    by_cart = sorted([q for q in queries if q["add_to_cart"] > 0], key=lambda x: -x["add_to_cart"])
    return {
        "period": period,
        "warnings": warnings[:12],
        "raw_rows": len(rows),
        "unique_queries": len(queries),
        "by_orders": by_orders[:80],
        "by_cart": by_cart[:80],
        "sum_orders": sum(q["orders"] for q in by_orders),
        "sum_cart": sum(q["add_to_cart"] for q in by_cart),
    }
